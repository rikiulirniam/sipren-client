...
function Sidebar() {
  ...
  useEffect(() => {
    const updateDate = () => {
      ...
      // Hitung total menit dari jam 00:00
      const totalMinutes = hour * 60 + minute;

      // Hitung jam ke-berapa (dimulai dari jam 07:00)
      const periodStartMinutes = 7 * 60; // 7:00 dalam menit
      const periodEndMinutes = 16 * 60; // 16:00 dalam menit
      let currentPeriod = "-";

      if (
        totalMinutes >= periodStartMinutes &&
        totalMinutes < periodEndMinutes
      ) {
        currentPeriod =
          Math.floor((totalMinutes - periodStartMinutes) / 45) + 1;
      } else if (totalMinutes >= periodEndMinutes) {
        currentPeriod = "-"; // Jika melewati jam 16:00
      }
      ...
      setDate({
        ...
        month: String(now.getMonth() + 1).padStart(2, "0"), // JavaScript months are 0-based
        ...
        <div className="navbar">
          <nav>
            <ul>
              ...
              <li className={pathname.includes("/data_user") ? "active" : ""}>
                <Link to="/data_user">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="36"
